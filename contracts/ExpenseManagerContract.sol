...
contract ExpenseManagerContract {
    ...
    function deposit(uint _amount, string memory _reason) public payable validAmount(_amount) {
    ...
    function withdraw(uint _amount, string memory _reason) public validAmount(_amount) sufficientBalance(_amount) {
        balances[msg.sender] -= _amount;
        transactions.push(
            Transaction(msg.sender, _amount, _reason, block.timestamp)
        );
